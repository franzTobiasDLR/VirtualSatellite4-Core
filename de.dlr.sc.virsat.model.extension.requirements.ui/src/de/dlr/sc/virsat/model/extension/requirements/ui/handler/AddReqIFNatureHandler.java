...
import de.dlr.sc.virsat.model.extension.requirements.reqif.builder.ReqIFNature;
	...
	public Object execute(ExecutionEvent event) throws ExecutionException {
		...
		if (selection instanceof TreeSelection) {
			...
			if (select.getFirstElement() instanceof IProject) {
				...
				try {
					addProjectNature(project, ReqIFNature.NATURE_ID);
				} catch (CoreException e) {
					Activator.getDefault().getLog().log(new Status(Status.ERROR, Activator.getPluginId(), "Could not add ReqIF nature to project", e));
				}
			}
		}
		return null;
